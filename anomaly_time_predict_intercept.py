...
def calculate_anomaly_time_all_df(path_to_probability, path_to_anomaly_time):
    ...
    if source_data == "clickhouse":
        ...
        df_clickhouse_csv = client.query_df(f"{config_json['paths']['database']['clickhouse']['original_csv_query']}")
        df_clickhouse_csv['timestamp'] = df_clickhouse_csv['timestamp'].astype('string')
        df_csv = pd.DataFrame()
        df_csv['timestamp'] = df_clickhouse_csv['timestamp'].copy(deep=True)
        client.close()
        ...
        df_csv['timestamp'] = df_csv['timestamp'].astype('string')
    ...
    for index, row in df_csv.iterrows():
        if pd.isna(row['P']) or not(pd.notna(row['P'])):
            ...
            freeze = False
        df_prediction_time.loc[index, 'P'] = row['P']
        df_prediction_time.loc[index, 'N'] = row['N']
        df_prediction_time.loc[index, 'potential'] = row['potential']

        # если уже в аномалии
        if row['P'] >= config_json['model']['P_pr'] * 100:
            print(row['timestamp'], row['P'], "ANOMALY")
            df_prediction_time.loc[index, 'anomaly_time'] = "0"
            df_prediction_time.loc[index, 'anomaly_date'] = row['timestamp'][:len(row['timestamp']) - 3]

            # Критерий по достижению вероятности
            delta_tau_P += 1
            # Критерий по достижению прогнозируемого времени
            delta_tau_T = 0
            df_prediction_time.loc[index, 'KrT'] = "0"
            if (delta_tau_P > (config_json['model']['delta_tau_P']*config_json['number_of_samples'])) and (not freeze):
                df_prediction_time.loc[index, 'KrP'] = "1"
            else:
                df_prediction_time.loc[index, 'KrP'] = "0"
            # Критерий по достижению прогнозируемого времени
            '''if delta_tau_T > 144:
            ...
            if index > regress_days:
                ...
                if (model.coef_[0] > 0) and (((root - float(index)) / config_json['number_of_samples']) < 720):
                    ...
                    df_prediction_time.loc[index, 'anomaly_time'] = \
                        0.0 if max(0, (root - float(index)) / config_json['number_of_samples']) == 0 else (
                                (root - float(index)) / config_json['number_of_samples'])
                    df_prediction_time.loc[index, 'anomaly_date'] = date_time + datetime.timedelta(
                        hours=(root - float(index)) / config_json['number_of_samples'])
                    df_prediction_time.loc[index, 'anomaly_date'] = df_prediction_time.iloc[index][
                    ...
                    if (datetime.timedelta(hours=df_prediction_time.iloc[index]['anomaly_time']) <=
                        ...
                        if delta_tau_T > (config_json['model']['delta_tau_T'] * config_json['number_of_samples'])\
                                and (not freeze):
                            df_prediction_time.loc[index, 'KrT'] = "1"
                        else:
                            df_prediction_time.loc[index, 'KrT'] = "0"
                    else:
                        delta_tau_T = 0
                        df_prediction_time.loc[index, 'KrT'] = "0"
                    print(row['timestamp'], row['P'], (root - float(index)) / config_json['number_of_samples'])
                else:
                    # либо вероятно падает либо до нее > 3 месяцев
                    df_prediction_time.loc[index, 'anomaly_time'] = "NaN"
                    df_prediction_time.loc[index, 'anomaly_date'] = "NaN"
                    df_prediction_time.loc[index, 'KrT'] = "0"
                    delta_tau_T = 0
