...
- potential_analyze_period_json.py: вычисление потенцила и loss по группам;
...
`table_potential_probability`: наименование csv файла-таблцы распределения потенцила вероятности (`заполнять без расширения с постфиксом: _`).
...
`hist_potential_probability`: наименование png файла с гистограммой распределения потенцила и вероятсности (`заполнять без расширения с постфиксом: _`).
