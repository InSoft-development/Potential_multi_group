...
`csv_truncate_by_power`: наименование csv файла с срезами, прошедшими отсечку по мощности.
...
8. При необходимости согласно конфигурационному файлу запускается  процедура create_online_table(), создающая таблицы в БД;
9. Запуск онлайн режима с помощью процедуры `online_sochi()`:
