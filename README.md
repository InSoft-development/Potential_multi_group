...
3. Инициализируются словари KKS датчиков, точек, таблиц распределения потенциала-вероятности, счетчиков выдержки сигнализаций, критериев срабатывания сигнализаций и окна регрессии по ключу группы;
4. Выполнение функции `kks_online_mode(approxlist, group)`: добавление KKS датчиков в словарь по ключу группы;
5. Выполнение функции `points_load_online_mode(group)`: добавление точек, определенных на этапе обучения, в словарь по ключу группы;
6. Выполнение функции `potential_probability_load_online_mode(group)`: добавление таблиц потенциал-вероятность, определенных на этапе обучения, в словарь по ключу группы;
7. При необходимости согласно выставленным настройкам в конфигурационном файле запускается процедура create_online_table(), создающая таблицы в БД;
8. Запуск онлайн режима с помощью процедуры `online_sochi()` до остановки пользователем скрипта:
		8.1. Инициализация подключения к БД clickhouse;
		8.2. Запрос к БД на получение ненормализованной и необъединенной последней строки из таблицы;
		8.3. Отсечка по мощности строки:
		...
		8.4. Для каждой группы определяется потенциал и loss с помощью функции `potentials_analyse_online_mode(data, nums, points)`. Если из-за значения мощности строку следует заморозить, то из БД запрашивается предпоследняя строка с потенциалом, loss и вероятностью, которая становится замороженной строкой;
		8.5. Вычисление времени до аномалии;
		8.6. Запись в таблицы БД loss по датчикам группы и потенциала, вероятности, времени до аномалии, сигнализаций KrP и KrT.
9. Остановка работы скрипта или повторение пункта 9 алгоритма.
